...
export default {
	findAll_features_c: async (req: any, res: any) => {
		...
		try {
			...
			if (features) {
				return res.status(200).send({ message: 'Features Found', data: features });
			}
	...
	findByPathname_features_c: async (req: any, res: any) => {
		...
		try {
			...
			console.log({ feature });
			if (feature) {
				return res.status(200).send({ message: 'Feature Found', data: feature });
			}
	...
	update_features_c: async (req: any, res: any) => {
		...
		try {
			...
			if (feature) {
				return res.status(200).send({ message: 'Feature Updated', data: feature });
			}

...
import { hslToHex, toCapitlize } from '../../utils/helper_functions';
...
const MonthlyExpensesPage = (props) => {
	...
	const data = {
		labels: monthly_income.map((month) => toCapitlize(month.month)),
		datasets: [
			{
				label: 'Income',
	...
	// 			backgroundColor: monthly_income.map((item) => {
	// 				num_2 += multiplier;
	// 				let color = hslToHex(num_2, 100, 50);
	// 				return color;
	// 			}),
	// 			color: 'white'
	// 		},
	...
	// 				if (item.profit > 0) {
	// 					num_3 += multiplier;
	// 					let color = hslToHex(num_3, 100, 50);
	// 					return color;
	// 				} else {
	// 					num_3 += multiplier;
	// 					let color = hslToHex(360, 100, 50);

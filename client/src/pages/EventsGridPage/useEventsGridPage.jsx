...
import { useChipsQuery, useEventsGridQuery } from "../../api/allRecordsApi";
import * as API from "../../api";
import { toSnakeCase } from "./eventGridPageHelpers";
import {
  setSelectedTags,
  setSelectedChip,
  setCategory,
  ...
  setSearch,
  updateFilters,
} from "./eventsGridPageSlice";

export const useEventsGridPage = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const dispatch = useDispatch();

  const { selectedTags, selectedChip, category, sort, search } = useSelector(state => state.events.eventsGridPage);
  ...
  }, [events, current_user?.isWholesaler]);

      ...
      <div className="profile-orders profile_orders_container" style={{ width: "100%" }}>
        ...
        <Loading loading={loading} error={error}>
          ...
          <div className="product_big_screen">
            {block_list_view &&
              orders &&
              orders.map((order, index) => <OrderItemD key={index} determine_color={determine_color} order={order} />)}
          </div>
          <div className="product_small_screen none column">
            {orders && orders.map((order, index) => <OrderItemM key={index} determine_color={determine_color} order={order} />)}
          </div>
        </Loading>
      </div>
    </div>

...
import EventPageHead from "./components/EventPageHead";
import { EditEventModal } from "../EventsPage/components";
import EventPageLoading from "./components/EventPageLoading";
import EventProtectionDetails from "../../shared/EventProtectionDetails/EventProtectionDetails";
...
import { openEditEventModal } from "../EventsPage/productsPageSlice";
...
import { setQuantity } from "./eventPageSlice";
...
const EventPage = () => {
  ...
  const { customizedEvent, current_user, my_cart, productPageLoading, product, isAddonChecked } = useEventPage();
  const [validationErrors, setValidationErrors] = useState({});

  const validateOptions = () => {
    const errors = {};
    customizedEvent.currentOptions?.forEach((option, index) => {
      if (!option.isAddOn && !customizedEvent.selectedOptions[index]?.name) {
        errors[index] = `Please select a ${option.name}`;
      } else if (option.isAddOn && isAddonChecked && !customizedEvent.selectedOptions[index]?.name) {
  ...
  const handleAddToCart = () => {
    if (validateOptions()) {
      dispatch(API.addToCart({ cart: my_cart, cartItem: customizedEvent, type: "add_to_cart" }));
    }
  };
  ...
  return (
    <Box>
      <EventPageHead product={product} />
      <EventPageLoading loading={productPageLoading}>
        {product && (
          <>
              ...
              {current_user?.isAdmin && (
                <Box className="br-10px">
                  <GLButtonV2
                    variant="contained"
                    color="secondary"
                    onClick={e => dispatch(openEditEventModal(product))}
                  >
                    Edit Event
                  </GLButtonV2>
                </Box>
            ...
            <Container maxWidth="xl">
              <Grid container spacing={2}>
                <Grid item xs={12} sm={12} md={6} lg={6}>
                  <EventImages images={customizedEvent?.images} originalImages={customizedEvent?.original_images} />
                </Grid>
                ...
                <Grid item xs={12} sm={12} md={6} lg={6}>
                  ...
                  <Typography variant="body1" gutterBottom mt={2} mb={2}>
                    {customizedEvent.short_description}
                  </Typography>
                  <CompatibleChips chips={customizedEvent.chips} />
                  {customizedEvent.currentOptions?.map((option, index) => (
                    <Box key={index}>
                      <CustomizationOption
                        index={index}
                        option={option}
                        selectedOption={customizedEvent?.selectedOptions[index]}
                  ...
                  <Typography variant="subtitle1">
                    Quantity {customizedEvent.set_of ? `(Set of ${customizedEvent.set_of})` : ""}
                  </Typography>
                  <GLSelect
                    value={customizedEvent?.quantity}
                    onChange={e => dispatch(setQuantity(e.target.value))}
                    placeholder="Select Quantity"
                    size="small"
                    options={[...Array(customizedEvent.max_quantity).keys()].map(value => ({ name: value + 1 }))}
                  ...
                  <Box mt={2}>
                    ...
                    >
                      {determineInStock(customizedEvent)}
                    </GLButtonV2>
                  </Box>
                </Grid>
              </Grid>
            ...
            <Box mt={4}>
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} mt={!product?.hero_video?.hidden && product?.hero_video?.video ? -4 : 0}>
                  <EventProtectionDetails transparent={false} />
                </Grid>
                ...
                <Grid container spacing={2}>
                  ...
                  {product?.product_support && !product?.product_support.hidden && (
                    <Grid item xs={12} id="support">
                      <EventSupport productSupport={product.product_support} />
                    </Grid>
                  )}
                  ...
                  <Grid item xs={12}>
                    <RecentlyViewed currentEvent={product} />
                  </Grid>
                </Grid>
              </Container>
            </Box>

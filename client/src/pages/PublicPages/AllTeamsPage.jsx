            ...
            <div className="product_big_screen">
              {teams && (
                <ul className="products" style={{ marginTop: 0, textDecoration: "none" }}>
                  {teams.map(
                    (team, index) => !team.hidden && <Team size="300px" key={index} team={team} category={props.match.params.category} />
                  )}
                </ul>
              )}
            </div>
            ...
            <div className="product_small_screen none">
              {teams && (
                <ul className="products" style={{ marginTop: 0, textDecoration: "none" }}>
                  {teams.map(
                    (team, index) =>
                      !team.hidden && <TeamSmallScreen size="300px" key={index} team={team} category={props.match.params.category} />
                  )}
                </ul>
              )}
            </div>

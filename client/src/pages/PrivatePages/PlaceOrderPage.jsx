...
import {
  ShippingChoice,
  ShippingSpeed,
} from "../../components/SpecialtyComponents/ShippingComponents";
...
const PlaceOrderPage = props => {
  ...
  const [ show_promo_code_input_box, set_show_promo_code_input_box ] = useState(
    true
  );
    ...
    () => {
      ...
      if (clean) {
        if (shipping && Object.keys(shipping).length > 0) {
          ...
          const package_volume = cartItems.reduce(
            (a, c) => a + c.package_volume,
            0
          );
  ...
  const choose_shipping_rate = (rate, speed) => {
    setShippingPrice(
      parseFloat(rate.retail_rate || rate.rate) + packaging_cost
    );
  ...
  const get_tax_rates = async () => {
    ...
    const result = state_names.find(obj => {
      return (
        obj.short_name === shipping.state || obj.long_name === shipping.state
      );
    });
    ...
    if (promo) {
      ...
      if (show_message) {
        ...
        if (promo.percentage_off) {
          ...
          setItemsPrice(
            items_price -
              (items_price - promo_excluded) * (promo.percentage_off / 100)
          );
          setTaxPrice(
            tax_rate *
              (items_price -
        ...
        if (promo.percentage_off) {
          set_show_message(
            `${promo.promo_code.toUpperCase()} ${promo.percentage_off}% Off`
          );
        } else if (promo.amount_off) {
          set_show_message(
            `${promo.promo_code.toUpperCase()} $${promo.amount_off} Off`
          );
        }
        // else  {
  ...
  return (
    <div>
      <Helmet>
        ...
        <link
          rel="canonical"
          href="https://www.glow-leds.com/secure/checkout/placeorder"
        />
        <meta
          property="og:url"
          content="https://www.glow-leds.com/secure/checkout/placeorder"
        />
      </Helmet>
      {decide_steps()}
      <LoadingPayments

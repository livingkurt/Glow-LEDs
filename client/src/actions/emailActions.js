...
const email_registration = (details) => async (dispatch) => {
  try {

    dispatch({ type: REGISTRATION_EMAIL_REQUEST });
    const { data } = await axios.post("/api/emails/register", details);
...
const email_order = (details) => async (dispatch) => {
  try {

    dispatch({ type: ORDER_EMAIL_REQUEST });
    const { data } = await axios.post("/api/emails/order", details);
...
const email_shipping = (order_details) => async (dispatch) => {
  try {

    dispatch({ type: SHIPPING_EMAIL_REQUEST });
    const { data } = await axios.post("/api/emails/shipping", order_details);

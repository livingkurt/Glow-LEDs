...
export const addToCart = (cart_item_1: any) => async (
  ...
  try {
    ...
    if (!cart_item_1.product && cart_item_1.pathname && cart_item_1.qty) {
      const same_item = cartItems.find(
        (item: any) => item.pathname === cart_data.pathname
      );
...
export const listCarts = (query: any) => async (
  dispatch: (arg0: { type: string; payload?: any }) => void
) => {
  try {
...
export const detailsCart = (cart_id: string) => async (
  dispatch: (arg0: { type: string; payload: any }) => void
) => {
  try {

  ...
  async ({ affiliate, profile }, { dispatch, rejectWithValue, getState }) => {
    try {
      ...
      if (newAffiliate) {
        ...
        console.log({ user: { _id: current_user._id, affiliate: data.newAffiliate._id }, profile, data });
        if (profile) {
...
// import { handleTokenRefresh } from "./axiosInstance";
// import { showStripeAccountLink } from "src/slices/affiliateSlice";
...
//         }
//         console.log({ accountLink: data.accountLink });
//         dispatch(showStripeAccountLink({ link: data.accountLink }));
//         return data;
//       } else {
//         const { data } = await axios.put(`/api/affiliates/${affiliate._id}`, affiliate);

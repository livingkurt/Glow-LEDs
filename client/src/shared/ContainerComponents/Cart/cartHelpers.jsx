...
export const useOutsideAlerter = (ref, dispatch) => {
  useEffect(() => {
    function handleClickOutside(event) {
      if (ref.current && !ref.current.contains(event.target) && event.target.className !== "qty_select_dropdown w-100per") {
...
export const checkoutHandler = (dispatch, history, props, cartItems, current_user, closeMenu) => {

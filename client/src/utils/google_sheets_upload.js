...
export const top_code_usage_upload = async (year, month) => {
  ...
  try {
    ...
    const sorted_last_months_rows = last_months_rows.affiliates.sort((a, b) =>
...
export const top_earner_upload = async (year, month) => {
  // google_sheets_json.private_key = config.REACT_APP_REACT_APP_GOOGLE_SHEETS_PRIVATE;
  ...
  try {
    ...
    const sorted_total_rows = total_rows.affiliates.sort((a, b) =>
      parseFloat(a.Revenue) > parseFloat(b.Revenue) ? -1 : 1
    );
...
export const affiliate_revenue_upload = async (position, year, month, google_sheet) => {
  // google_sheets_json.private_key = config.REACT_APP_REACT_APP_GOOGLE_SHEETS_PRIVATE;
...
export const facebook_catalog_upload = async products => {
  // google_sheets_json.private_key = config.REACT_APP_REACT_APP_GOOGLE_SHEETS_PRIVATE;
  ...
  try {
    ...
    const new_rows = products.map((product, i) => {
...
export const current_products_upload = async products => {
  // google_sheets_json.private_key = config.REACT_APP_REACT_APP_GOOGLE_SHEETS_PRIVATE;
...
export const google_catalog_upload = async products => {
  // google_sheets_json.private_key = config.REACT_APP_REACT_APP_GOOGLE_SHEETS_PRIVATE;
  try {
    const { GoogleSpreadsheet } = require("google-spreadsheet");
...
export const orders_upload = async orders => {
  // google_sheets_json.private_key = config.REACT_APP_REACT_APP_GOOGLE_SHEETS_PRIVATE;
  ...
  try {
    ...
    const new_rows = orders.map((order, i) => {
      const id = order._id;
      ...
      const item_ids = order.orderItems.map(item => item._id).join(",");
      const item_names = order.orderItems.map(item => item.name).join(",");
      const item_prices = order.orderItems.map(item => item.price).join(",");
      const item_quantities = order.orderItems.map(item => item.qty).join(",");
      const item_size = order.orderItems.map(item => item.size).join(",");
      const item_color = order.orderItems.map(item => item.color).join(",");

...
import {
  removeDuplicates,
  calculate_affiliate_usage,
  calculate_sponsor_usage,
  toCapitalize,
...
export const top_code_usage_upload = async (year: number, month: string) => {
  ...
  try {
    ...
    const doc = new GoogleSpreadsheet(
      "1iGUFKugqgZs6kwbgz_FgQzqa7ivFA7BZmvjTJvvyHyI"
    );
    ...
    const {
      data: last_months_rows,
    } = await API_Orders.affiliate_code_usage_orders_a({
      year,
      month,
      position: "",
    });
    ...
    const formated_total = removeDuplicates(
      sorted_total_rows,
      "Promo Code"
    ).map((affiliate: any) => {
    ...
    const formated_last_month = removeDuplicates(
      sorted_last_months_rows,
      "Promo Code"
    ).map((affiliate: any) => {
      return { ...affiliate, Revenue: `$${affiliate.Revenue}` };
    });
...
export const top_earner_upload = async (year: number, month: string) => {
  ...
  try {
    ...
    // const doc = new GoogleSpreadsheet('1qf9xryR0EPOCD0YkFQXqYioAxJRfWg6QFpdFwFTpErg');
    const doc = new GoogleSpreadsheet(
      "1HMi3HF1f_5mJZqievCYOfrOuDjQkZeNLD88DoCw7kl0"
    );
...
export const affiliate_revenue_upload = async (
  position: any,
  year: number,
  month: string,
  google_sheet: string
  ...
  try {
    ...
    const newSheet = await doc.addSheet({
      title: `${toCapitalize(month)} ${year} ${position
        ? toCapitalize(position)
        : "Affiliate"} Revenue`,
    });
  ...
  try {
    ...
    const new_rows = products.map((product: any, i: number) => {
      ...
      const sale_price = `${product.sale_price &&
        product.sale_price.toFixed(2)} USD`;
...
export const google_catalog_upload = async (products: any) => {
  ...
  try {
    ...
    // spreadsheet key is the long id in the sheets URL
    const doc = new GoogleSpreadsheet(
      "1V9vSROcN0RA-OFRGOIbvt_raXh3ZG2BYDY9DSOudaqU"
    );
...
export const orders_upload = async (orders: any) => {
  ...
  try {
    ...
    // const doc = new GoogleSpreadsheet('1qf9xryR0EPOCD0YkFQXqYioAxJRfWg6QFpdFwFTpErg');
    const doc = new GoogleSpreadsheet(
      "1dZd8vv3E_-9FSpruQAlWfI4TjkXI9hukt2mG88OQlXE"
    );
    // const doc = new GoogleSpreadsheet('1IS8GkQmPTsBcPM8qv0ifGIjTpeDSjkx4FuYA0ZmEq2o');
    ...
    const new_rows = orders.map((order: any, i: number) => {
      ...
      const item_names = order.orderItems
        .map((item: any) => item.name)
        .join(",");
      const item_prices = order.orderItems
        .map((item: any) => item.price)
        .join(",");
      const item_quantities = order.orderItems
        .map((item: any) => item.qty)
        .join(",");
      const item_size = order.orderItems
        .map((item: any) => item.size)
        .join(",");
      const item_color = order.orderItems

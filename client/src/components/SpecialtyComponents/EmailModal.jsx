...
const EmailModal = props => {
  ...
  const submitHandler = async e => {
    ...
    if (request.isValid) {
      ...
      const { data } = await API_Emails.send_email_subscription(
        email,
        promo.promo_code
      );
      localStorage.setItem(
        "popup",
        JSON.stringify({ date: today, email: true })
      );
      set_complete(true);
  ...
  return (
    <div
      ...
      className={`modal-floating ${complete
        ? "max-h-325px"
        : "max-h-300px"} max-w-500px fade_in ${width < 535 && "mh-auto-20px"}`}
    >
      <span
        ...
        onClick={() => {
          ...
          localStorage.setItem(
            "popup",
            JSON.stringify({ date: today, email: false })
          );
        }}
      >
        &times;
      ...
      {complete ? (
        <ul className="column jc-b ai-c p-20px">
          <li className="mb-2rem">
            <label
              className={`title_font ${width < 535
                ? "fs-20px lh-30px"
                : "fs-28px lh-40px"} ta-c jc-c `}
            >
          ...
          <li>
            <p className="p_descriptions fs-16px ta-c jc-c">
        ...
        <form onSubmit={submitHandler} className="modal-content-floating ">
          <div className="h-100per jc-b column">
            <label className="p_descriptions fs-16px ta-c jc-c">
              Come Into the Light
            </label>
            <label
              className={`title_font ${width < 535
                ? "fs-20px lh-30px"
                : "fs-30px lh-40px"} ta-c jc-c `}
            >

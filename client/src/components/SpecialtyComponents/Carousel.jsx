...
const Carousel = (props) => {
	...
	const get_products = async (category) => {
		...
		set_products(
			typeof data === 'object' && data.products.filter((product) => product.pathname !== props.product_pathname)
		);
		if (props.random) {
			set_products(
				typeof data === 'object' &&
					shuffle(data.products.filter((product) => product.pathname !== props.product_pathname))
			);
		}
		set_loading(false);
	};
